	...
	final class DGWT_WC_Ajax_Search {
		...
		public static function get_instance() {
			if ( !isset( self::$instance ) && !( self::$instance instanceof DGWT_WC_Ajax_Search ) ) {
				...
				$engine = self::$instance->engine = self::$instance->settings->get_opt('search_engine', 'native');
                ...
                switch($engine){
                    case 'native':
	                    new \DgoraWcas\Engines\WordPressNative\Search;
           ...
           if(empty($screen->id) || ($screen->id !== 'dashboard' && $screen->id !== 'plugins')){
            ...
            if(!get_transient('dgwt-wcas-php55-notice-dismiss')) {
                ...
                <div class="error">
                    <p>
                        <?php
                        printf(__("<b>AJAX Search for WooCommerce</b>:<br /> Your PHP version <b><i>%s</i></b> will not longer supported in the next plugin releases.",
                        ...
                        _e("If you cannot upgrade your PHP version yourself, you can send an email to your host.",
                            'ajax-search-for-woocommerce');
                        echo '<br /><br />';
                        echo '<span style="font-weight:bold; color: #dc3232">' . __('If you do not upgrade the php version, the next plugin release will not work!',
                                'ajax-search-for-woocommerce') . '</span>';
                        echo '<br />';
                        echo '<br />';
                        echo '<a href="' . esc_url(add_query_arg(array('dgwt-wcas-php55-notice' => 'dismiss'),
                                $_SERVER['REQUEST_URI'])) . '">' . __('Remind me again in week.',
		...
		private function constants() {
			...
			$this->define( 'DGWT_WCAS_VERSION', $plugin_data['Version'] );
		...
		public function admin_scripts() {
            ...
            wp_register_script( 'dgwt-wcas-admin-js', DGWT_WCAS_URL . 'assets/js/admin' . $min . '.js', array( 'jquery' ), DGWT_WCAS_VERSION );
